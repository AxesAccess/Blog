...
answers.columns = ["Score", "Answer"]

answers["Score"] = answers["Score"].apply(lambda x: "ABCDE".find(x) + 1)
```
...
df_final = df.melt(id_vars=index_vars, value_vars=value_vars, var_name="Question", value_name="Answer")
df_final.head()
```
...
df_final = df_final.merge(answers[["Answer", "Score"]], how="left")

df_final["Answer"] = df_final["Score"].apply(lambda x: "ABCDE".find(str(x)))
df_final["Type"] = df_final["Question"].apply(lambda x: re.sub(r"F\d+_(.+)", "\\1", x))
df_final["ID"] = df_final["Question"].apply(lambda x: re.sub(r"(F\d+_).+", "\\1", x))
