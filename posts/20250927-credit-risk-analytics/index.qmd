...
judged to be genuine, albeit high, observations.
...
Risk](02_models.html){target="_blank"}.

### 1. Logistic Regression (GLM) for Interpretability

I initially built a Logistic Regression model (GLM), which provides
highly interpretable coefficient estimates. Due to the nature of the
model and to avoid singularity issues, I proactively excluded
**`loan_grade`** and **`loan_int_rate`** (as they are likely set by the
bank based on pre-assessment).

For categorical features with high cardinality, such as `city` and
`country`, I used **one-hot encoding**. When running the full encoded
model, it proved highly complex. I then employed **stepwise selection
(MASS::stepAIC)**, an optimization technique that systematically adds or
removes variables to minimize the model’s AIC (Akaike Information
Criterion), yielding a balance between fit and simplicity.
...
features well, I initially trained this model on the full, cleaned
dataset (excluding only the two variables noted above).
...
maintained or slightly improved performance, reaching an AUC of
**0.861**.
...
for the highly complex portfolio simulation component of the dashboard,
while the GLM was retained for the more interpretable marginal effects
calculations.
...
The script calculates **Average Marginal Effects (AMEs)**. The logic
behind AMEs is that they estimate the change in the probability of
default ($P$) associated with a unit change in a predictor variable
($x_j$), calculated as $\beta_j \cdot p \cdot (1-p)$, where $p$ is the
average predicted probability. This output helps users understand the
true economic impact of each driver, not just the raw logistic
coefficient ($\beta_j$). The results showed clear impact; for example,
Home Ownership (OWN) had a standardized effect of -0.1707.
...
To power the **Portfolio Simulation** feature, I needed a massive,
...
realistic diversity, I applied **jitter** (random noise) to numeric
features like `person_age` and `loan_amnt`. Finally, I applied the
pre-trained XGBoost model to these 50,000 synthetic records to predict a
**`Predicted_PD`** for each one. This synthetic dataset was also
enhanced with categorization bins (e.g., `age_bin`, `income_bin`) to
enable interactive slicing and scenario testing within the dashboard.
...
via Power Query. The R visual is mainly for creating plots and have
...
impossible to adjust parameters interactively. To address this, I used
model coefficients in the default risk calculator, and the sum of the
products was then converted into a predicted probability of default by
applying the inverse-logit function, $1/(1 + \exp(-\alpha))$.

In the portfolio simulation I created DAX measures and slicers that
allow users to modify inputs, which then filter the pre-calculated
synthetic dataset.

The final dashboard was structured into three main sections:

-   **Portfolio Overview:** key statistics such as total borrowers,
    portfolio value, and overall default rate.
-   **Default Risk Calculator:** an interactive tool allowing users to
    ...
    default along with marginal effects for each factor. For an example
    borrower profile (e.g., Person income **67,000**, Loan amount
    **9,500**, **RENT** ownership, **VENTURE** intent), the predicted
    Probability of Default (PD) was **25.3%**.
-   **Portfolio Simulation:** a scenario analysis tool where users can
    ...
    and profits. The underlying XGBoost model metrics demonstrated
    exceptional classification ability, with a Sensitivity of **0.9831**
    and a Specificity of **0.7012**.
