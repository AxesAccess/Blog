...
summary(marginal_effects) |> kableExtra::kable()
```
...
summary(marginal_effects_unmatched) |> kableExtra::kable()
```
...
t_test_all_effect <- (unname(t_test_all$estimate[2] - t_test_all$estimate[1]) * 100) |> format(digits = 4)
t_test_effect <- (unname(t_test$estimate[2] - t_test$estimate[1]) * 100) |> format(digits = 4)
...
comparable new customers ($p = `r t_test_p_value`$).
