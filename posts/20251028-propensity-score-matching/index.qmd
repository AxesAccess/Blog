...
dataset](https://archive.ics.uci.edu/dataset/222/bank+marketing){target="_blank"}
from the UCI Machine Learning Repository to estimate the causal effect
of being previously contacted on the probability of subscribing to a
term deposit.

We’ll use the R package
[MatchIt](https://cloud.r-project.org/web/packages/MatchIt/index.html){target="_blank"}
to perform matching and evaluate balance.
...
#| fig-height: 7
#| fig-width: 7
#| fig-cap: "Distributions and correlations of numeric variables in the dataset."
...
being re-contacted and subscribing. We need to exclude `campaign` (number
of contacts performed during this campaign and for this client),
`contact` (contact communication type), `previous` (number of contacts
performed before this campaign), and `poutcome` (outcome of the previous
marketing campaign) to avoid leaking information about treatment
assignment.
...
```{r fig-balance-plot}
#| warning: false
#| fig-cap: "Plot of propensity scores before and after matching."
...
```{r fig-density-plots}
#| warning: false
#| fig-cap: "Density plots of variables before and after matching."
...
#| warning: false
#| fig-cap: "Love plot showing standardized mean differences before and after matching."
library(cobalt)
...
t_test_all_effect <- unname(t_test_all$estimate[2] - t_test_all$estimate[1]) * 100
t_test_effect <- unname(t_test$estimate[2] - t_test$estimate[1]) * 100
t_test_diff_pp <- t_test_all_effect - t_test_effect
t_test_diff <- (t_test_all_effect - t_test_effect) / t_test_all_effect * 100

t_test_all_effect <- t_test_all_effect |> format(digits = 4)
t_test_effect <- t_test_effect |> format(digits = 4)
t_test_diff_pp <- t_test_diff_pp |> format(digits = 4)
t_test_diff <- abs(t_test_diff)  |> format(digits = 4)
...
$`r t_test_all_effect`$ percentage points more likely to subscribe to a
term deposit. After controlling for demographics and financial variables
via propensity score matching, previously contacted customers were
$`r t_test_effect`$ percentage points more likely to subscribe than
comparable new customers ($p = `r t_test_p_value`$), which is
$`r t_test_diff_pp`$ percentage points ($`r t_test_diff`\%$) less than
...
```{r fig-subscription-rates}
#| echo: false
#| fig-cap: Subscription rates before and after matching.
#| fig-height: 4
#| fig-width: 6

library(ggpubr)

subscription_rates <- data.frame(
  Stage = c("Before Matching", "After Matching"),
  Treated = c(
    mean(dataset$outcome[dataset$treat == 1]),
    mean(matched_data$outcome[matched_data$treat == 1])
  ),
  Control = c(
    mean(dataset$outcome[dataset$treat == 0]),
    mean(matched_data$outcome[matched_data$treat == 0])
  )
)
subscription_rates$Stage <- subscription_rates$Stage |>
  factor(levels = c("Before Matching", "After Matching"))

subscription_rates_long <- subscription_rates |>
  pivot_longer(
    cols = c("Treated", "Control"),
    names_to = "Group", values_to = "SubscriptionRate"
  )


fig <- ggplot(subscription_rates_long, aes(x = Stage, y = SubscriptionRate, fill = Group)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.6) +
  labs(
    y = "Subscription Rate",
    x = element_blank(),
  ) +
  scale_y_continuous(labels = scales::percent_format(), limits = c(0, 0.27)) +
  theme_bw(base_size = 14) +
  scale_fill_brewer(palette = "Paired") +
  geom_bracket(
    xmin = 1 - 0.15, xmax = 2 - 0.15,
    y.position = max(subscription_rates_long$SubscriptionRate) * 1.1,
    label = paste0("Δ ", paste(round(diff(subscription_rates$Control)*100, 1), "percent. points")),
...
ggsave("image.svg", fig, width = 6, height = 4)
...
Ho D, Imai K, King G, Stuart E (2011). “MatchIt: Nonparametric
Preprocessing for Parametric Causal Inference.” *Journal of Statistical
Software*, *42*(8), 1-28. doi:10.18637/jss.v042.i08
